...
BEGIN
	...
	SET
		...
		target.turno = COALESCE(ENCRYPTBYKEY(KEY_GUID('EmpleadoLlave'), CONVERT(VARBINARY, source.turno)), target.turno),
	...
	INSERT INTO dbEmpleado.Empleado (legajoEmpleado, nombre, apellido, direccion, emailPersonal, emailEmpresa, cargo, idSucursal, turno, cuil, cuilHash, fechaAlta)
	SELECT 
		source.legajo,
		...
		source.idSucursal,
		ENCRYPTBYKEY(KEY_GUID('EmpleadoLlave'), CONVERT(VARBINARY, source.turno)),
